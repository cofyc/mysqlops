...
BACKUP_LOCK="/var/run/mysqlops.backup.lock"
BACKUP_TIME=$(date +%s)
...
if ! `echo 'exit' | mysql`; then
    kube::log::error_exit "failed to connect to mysql server."
fi

if ! `which innobackupex &>/dev/null`; then
    kube::log::error_exit "innobackupex not found, please install Percona Xtrabackup."
fi
...
if should_create_incr_backup "$FULL_LATEST" "$BACKUP_TIME"; then
    kube::log::status "New increment backup."
    ...
    kube::log::status "New full backup..."
    ...
    # Finally, the binary kube::log::info position will be printed to STDERR and innobackupex will exit returning 0 if all went OK.
    if [ $? -eq 0 ]; then
        kube::log::status "Full backup ok."
        echo ""
        echo "------ LAST OUTPUT FROM $BACKUP_INNOBACKUPEX_FULL_LOGFILE ------"
        tail -n 5 $BACKUP_INNOBACKUPEX_FULL_LOGFILE
        echo ""
    else
        kube::log::status "Full backup failed."
        echo ""
...
for f in $(find $BACKUP_FULL_DIR -mindepth 1 -maxdepth 1 -printf "%P\n" | sort); do
    ...
    if should_clean "$dir" $BACKUP_TIME; then
        ...
        kube::log::status "Compressing $BACKUP_FULL_DIR/$f to $BACKUP_FULL_DIR/$dir.tar.gz..."
        if [[ "$f" =~ \.tar$ ]]; then
        ...
        if [ $? -eq 0 ]; then
            kube::log::status "Compressing ok."
        fi
    fi
done
