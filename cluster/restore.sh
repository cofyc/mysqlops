...
RESTORE_LOCK="/var/run/mysqlops.restore.lock"
...
if ! test -d "$RESTORE_DIR"; then
    kube::log::error_exit "restore dir '$RESTORE_DIR' does not exist."
fi

test -d "$WORKING_DIR" || mkdir -p "$WORKING_DIR"

if ! is_dir_empty "$WORKING_DIR"; then
    kube::log::error_exit "working dir '$WORKING_DIR' is not empty."
fi

if ! is_dir_empty "$DATADIR"; then
    kube::log::error_exit "datadir '$DATADIR' is not empty"
fi

RESTORE_INCR_DIRS=()

if is_incr_backup "$RESTORE_DIR"; then
    kube::log::status "Restore incremental backup from: $RESTORE_DIR"
    ...
    kube::log::status "Restore full backup from: $RESTORE_DIR"
fi

kube::log::status "RESTORE_BASE_DIR: $RESTORE_BASE_DIR"

# Syncing base dir to working dir.
# Notes: base dir may be gzipped.
if test -d "$RESTORE_BASE_DIR"; then
    kube::log::status "Rsyncing $RESTORE_BASE_DIR to $WORKING_DIR..."
    rsync -av "$RESTORE_BASE_DIR/" "$WORKING_DIR/" &> "$WORKING_DIR/rsync.log"
    if [ $? -eq 0 ]; then
        kube::log::status "Rsyncing ok."
    else
        kube::log::error_exit "Rsyncing failed."
    fi
elif test -f "$RESTORE_BASE_DIR.tar.gz"; then
    gziped_tar="$RESTORE_BASE_DIR.tar.gz"
    kube::log::status "Extracting $gziped_tar to $WORKING_DIR..."
    tar -xvf "$gziped_tar" -C "$WORKING_DIR" --strip 1 &> "$WORKING_DIR/tar.extract.log"
    if [ $? -eq 0 ]; then
        kube::log::status "Extracting done."
    else
        kube::log::error_exit "Extracting failed."
    fi
else
    kube::log::error_exit "Could not find contents of $RESTORE_BASE_DIR."
fi
...
if is_incr_backup "$RESTORE_DIR"; then
    ...
    kube::log::status "Replaying on the committed transactions on base dir..."
    ...
    if [ $? -eq 0 ]; then
        kube::log::status "Replaying ok."
    else
        kube::log::error_exit "Replaying failed."
    fi
    for incr in "${RESTORE_INCR_DIRS[@]}"; do
        if [[ "$incr" == "$RESTORE_INCR_DIR_LAST" ]]; then
            kube::log::status "Replaying on the committed transactions on incremental dir ($incr, the last)..."
            echo innobackupex $MYSQL_OPTIONS \
                --apply-log \
                $APPLY_LOG_OPTIONS $WORKING_DIR
            innobackupex $MYSQL_OPTIONS \
            ...
            kube::log::status "Replaying on the committed transactions on incremental dir ($incr)..."
            echo innobackupex $MYSQL_OPTIONS \
                --apply-log --redo-only \
                $APPLY_LOG_OPTIONS $WORKING_DIR
            innobackupex $MYSQL_OPTIONS \
...
kube::log::status "Preparing backup to roll back the uncommitted transactions..."
echo innobackupex $MYSQL_OPTIONS \
...
if [ $? -eq 0 ]; then
    kube::log::status "Preparing ok."
else
    kube::log::error_exit "Preparing failed."
fi

kube::log::status "Restoring..."
...
if [ $? -eq 0 ]; then
    kube::log::status "Restoring ok."
else
    kube::log::error_exit "Restoring error."
fi

chown -R mysql:mysql "$DATADIR"

kube::log::status "$RESTORE_DIR restored to $DATADIR successfully."
kube::log::status "You are able to start mysql now."
kube::log::status "Working directory '$WORKING_DIR' can be removed now."
